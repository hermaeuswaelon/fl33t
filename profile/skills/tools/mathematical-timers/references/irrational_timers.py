...
CONSTANTS = {
    ...
    'e_pi': math.e ** math.pi,        # e^π ≈ 23.14069
    'pi_e': math.pi ** math.e,        # π^e ≈ 22.45916
    'phi_pi': ((1 + math.sqrt(5)) / 2) * math.pi,  # φ × π ≈ 5.0832
    'pi_phi': math.pi * ((1 + math.sqrt(5)) / 2),   # π × φ (alias)
}

ALL_CONSTANTS = list(CONSTANTS.keys())

# ─── Constant Groups ───
TRANSCENDENTAL = ['pi', 'e', 'e_pi', 'pi_e', 'phi_pi']
ALGEBRAIC = ['phi', 'sqrt2', 'sqrt3', 'sqrt5', 'sqrt_pi']
LOGARITHMIC = ['ln2', 'ln10']
SPECIAL = ['gamma', 'zeta3', 'catalan']

# ─── State Persistence ───
DEFAULT_STATE_FILE = "/tmp/irrational_timer_state.json"
...
class IrrationalTimer:
    ...
    state: TimerState = field(default_factory=TimerState)
    ...
    def wait(
        ...
        if const not in CONSTANTS:
            raise ValueError(f"Unknown constant: {const}. Available: {list(CONSTANTS.keys())}")
    ...
    def wait_chaos(
        ...
        const = random.choice(list(CONSTANTS.keys()))
        if verbose:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        print("\n2. Wait with π:")
        timer.wait(constant='pi', min_sec=0.1, max_sec=0.5)
        
        print("\n3. Chaos mode:")
        timer.wait_chaos(min_sec=0.1, max_sec=0.5)
        
        print("\n4. Fibonacci sequence:")
        timer.wait_fibonacci(3, base_constant='pi', min_sec=0.1, max_sec=0.5)
        
        print("\n5. Prime sequence:")
        timer.wait_primes(3, constant='e', min_sec=0.1, max_sec=0.5)
